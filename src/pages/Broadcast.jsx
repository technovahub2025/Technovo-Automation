...
const Broadcast = () => {
    ...
    const loadTemplates = async () => {
        try {
            ...
            const templateList = Array.isArray(result) ? result : Array.isArray(result?.data) ? result.data : [];
            const templateMap = templateList.reduce((acc, template) => {
                const name = String(template?.name || '').trim();
                if (!name) return acc;
                acc[name] = String(template?.content || template?.body || template?.message || template?.text || '');
                return acc;
            }, {});
    ...
    const handleFileUpload = async (event) => {
        ...
        if (file) {
            ...
            const nextRecipients = Array.isArray(result?.recipients)
                ? result.recipients
                : Array.isArray(result?.data?.recipients)
                    ? result.data.recipients
                    : [];
            if (result.success || nextRecipients.length > 0) {
                setRecipients(nextRecipients);
            } else {
                alert('Failed to process CSV: ' + (result.message || result.error || 'Unknown error'));
            }
        }
    };
    ...
    const handleSendBroadcast = async () => {
        ...
        const bulkData = {
            messageType,
            recipients,
            broadcast_name: broadcastName || `Bulk Send - ${new Date().toLocaleString()}`,
            name: broadcastName || `Bulk Send - ${new Date().toLocaleString()}`,
        ...
        const result = await whatsappService.sendBulkMessages(bulkData);

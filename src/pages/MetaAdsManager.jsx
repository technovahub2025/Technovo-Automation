...
const MetaAdsManager = () => {
  ...
  const lockWizardPrimaryButton = (node) => {
    if (!node) return;
    node.style.setProperty("background", "linear-gradient(135deg, #0c6c58 0%, #149470 100%)", "important");
    node.style.setProperty("background-color", "#0c6c58", "important");
    node.style.setProperty("background-image", "linear-gradient(135deg, #0c6c58 0%, #149470 100%)", "important");
    node.style.setProperty("color", "#ffffff", "important");
    node.style.setProperty("border-color", "transparent", "important");
    node.style.setProperty("box-shadow", "0 12px 26px rgba(12, 108, 88, 0.22)", "important");
  ...
  const activeAdPreview =
    adPreview?.previews?.find((item) => item.key === selectedAdPreviewPlacement) ||
    adPreview?.previews?.[0] ||
    null;
  ...
  return (
    <div className="meta-ads-saas-page">
      ...
      {adPreview ? (
        <div className="meta-preview-backdrop" onClick={closeAdPreview} role="presentation">
          <div className="meta-preview-panel" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-label="Meta ad preview">
            <div className="meta-preview-head">
              <div>
                <span className="meta-eyebrow">Ad Preview</span>
                <h2>{adPreview.adName}</h2>
                <p>Rendered from Meta Ad ID <strong>{adPreview.adId}</strong>.</p>
              </div>
              <button type="button" className="meta-close-button" onClick={closeAdPreview}>
                Close
              </button>
            </div>

            <div className="meta-preview-tabs">
              {adPreviewPlacements.map((placement) => (
                <button
                  key={placement.key}
                  type="button"
                  className={`meta-preview-tab ${selectedAdPreviewPlacement === placement.key ? "active" : ""}`}
                  onClick={() => setSelectedAdPreviewPlacement(placement.key)}
                >
                  {placement.label}
                </button>
              ))}

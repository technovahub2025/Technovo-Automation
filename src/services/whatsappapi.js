...
api.interceptors.request.use(
      ...
      try {
        const sessionUser =
          typeof sessionStorage !== "undefined"
            ...
            : null;
        return JSON.parse(
          localStorage.getItem("user") || sessionUser || "null",
        );
      } catch {
        return null;
      }
  ...
  (error) => {
    return Promise.reject(normalizeError(error, "Request setup failed"));
  },
);
...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    return Promise.reject(normalizeError(error, "API request failed"));
  },
);
...
export const apiClient = {
  ...
  getConversations: (params = {}) =>
    api.get("/conversations", { params, timeout: LONG_TIMEOUT_MS }),
   ...
   */
  getConversation: (id) =>
    api.get(`/conversations/${id}`, { timeout: LONG_TIMEOUT_MS }),
  ...
  createConversation: (data) =>
    api.post("/conversations", data, { timeout: LONG_TIMEOUT_MS }),
  ...
  updateConversation: (id, data) =>
    api.put(`/conversations/${id}`, data, { timeout: LONG_TIMEOUT_MS }),
  ...
  markConversationAsRead: (id) =>
    api.put(`/conversations/${id}/read`, undefined, {
   ...
   */
  sendMessage: (data) =>
    api.post("/messages/send", data, { timeout: LONG_TIMEOUT_MS }),
  ...
  uploadBroadcastTemplateMedia: async (file) => {
    ...
    try {
      const response = await uploadApi.post(
        "/messages/template-header-media",
        formData,
        {
          headers: {
      ...
      throw normalizeError(error, "Template header upload failed");
    }
  },
   ...
   */
  getContacts: (params = {}) =>
    api.get("/contacts", { params, timeout: LONG_TIMEOUT_MS }),
   ...
   */
  createContact: (data) =>
    api.post("/contacts", data, { timeout: LONG_TIMEOUT_MS }),

  scanContactCard: (file) => {
    const formData = new FormData();
    formData.append("image", file);
    return api.post("/contacts/scan-card", formData, {
      timeout: LONG_TIMEOUT_MS,
      headers: {
        "Content-Type": "multipart/form-data",
      },
  ...
  markContactWhatsAppOptOut: (id, data = {}) =>
    api.post(`/contacts/${id}/whatsapp-opt-out`, data, {
      timeout: LONG_TIMEOUT_MS,
    }),

  getContactWhatsAppStatus: (id) =>
    api.get(`/contacts/${id}/whatsapp-status`, { timeout: LONG_TIMEOUT_MS }),

  getContactWhatsAppConsentAudit: (id) =>
    api.get(`/contacts/${id}/whatsapp-consent-audit`, {
      timeout: LONG_TIMEOUT_MS,
    }),
   ...
   */
  deleteContact: (id) =>
    api.delete(`/contacts/${id}`, { timeout: LONG_TIMEOUT_MS }),

  // ============ MISSED CALLS ============
  getMissedCalls: (params = {}) =>
    api.get("/missedcalls", { params, timeout: LONG_TIMEOUT_MS }),
  resolveMissedCall: (id, payload = {}) =>
    api.put(`/missedcalls/${id}/resolve`, payload, {
      timeout: LONG_TIMEOUT_MS,
    }),
  runMissedCallNow: (id) =>
    api.post(`/missedcalls/${id}/run-now`, undefined, {
      timeout: LONG_TIMEOUT_MS,
    }),
  getMissedCallSettings: () =>
    api.get("/missedcalls/settings", { timeout: LONG_TIMEOUT_MS }),
  updateMissedCallSettings: (data) =>
    api.put("/missedcalls/settings", data, { timeout: LONG_TIMEOUT_MS }),
  ...
  getTemplates: async (params = {}) => {
    try {
      return await api.get("/templates/meta", {
        params,
        timeout: LONG_TIMEOUT_MS,
      });
    } catch (error) {
      console.warn(
        "Meta template fetch failed, falling back to local templates:",
   ...
   */
  syncTemplates: () =>
    api.post("/templates/meta/sync", undefined, { timeout: LONG_TIMEOUT_MS }),
   ...
   */
  getTemplate: (id) =>
    api.get(`/templates/${id}`, { timeout: LONG_TIMEOUT_MS }),
   ...
   */
  createTemplate: (data) =>
    api.post("/templates", data, { timeout: LONG_TIMEOUT_MS }),
  ...
  updateTemplate: (id, data) =>
    api.put(`/templates/${id}`, data, { timeout: LONG_TIMEOUT_MS }),
   ...
   */
  deleteTemplate: (id) =>
    api.delete(`/templates/${id}`, { timeout: LONG_TIMEOUT_MS }),
  ...
  getCsvImportJob: (id) =>
    api.get(`/bulk/imports/${id}`, { timeout: LONG_TIMEOUT_MS }),

  getCsvImportJobs: (params = {}) =>
    api.get("/bulk/imports", { params, timeout: LONG_TIMEOUT_MS }),

  cancelCsvImportJob: (id) =>
    api.post(`/bulk/imports/${id}/cancel`, undefined, {
      timeout: LONG_TIMEOUT_MS,
    }),
  ...
  sendBulkMessages: (data) =>
    api.post("/bulk/send", data, { timeout: LONG_TIMEOUT_MS }),

  validateBroadcastAudience: async (data = {}) => {
    try {
      return await api.post("/bulk/validate-audience", data, {
        timeout: LONG_TIMEOUT_MS,
      });
    } catch (error) {
      ...
      if (
        status === 500 &&
        backendError.includes("buildPhoneCandidates is not defined")
      ) {
        return {
          status: 200,
          statusText: "OK",
      ...
      throw normalizeError(error, "Audience validation failed");
    }
  },

  syncMetaLeadConsentBatch: (data) =>
    api.post("/meta-ads/leads/sync-consent/batch", data, {
      timeout: LONG_TIMEOUT_MS,
    }),
   ...
   */
  getConversationContacts: (params = {}) =>
    api.get("/conversations/contacts/unique", {
      params,
      timeout: LONG_TIMEOUT_MS,
    }),
  ...
  getBroadcasts: (params = {}) =>
    api.get("/broadcasts", { params, timeout: LONG_TIMEOUT_MS }),

  getCampaignSelectionBroadcasts: (params = {}) =>
    api.get("/broadcasts/selection/campaigns", {
      params,
      timeout: LONG_TIMEOUT_MS,
    }),
   ...
   */
  getBroadcast: (id) =>
    api.get(`/broadcasts/${id}`, { timeout: LONG_TIMEOUT_MS }),
  ...
  getBroadcastReliabilitySummary: (params = {}) =>
    api.get("/broadcasts/analytics/reliability", {
      params,
      timeout: LONG_TIMEOUT_MS,
    }),

  retryFailedBroadcastRecipients: (id) =>
    api.post(`/broadcasts/${id}/retry-failed`, undefined, {
      timeout: LONG_TIMEOUT_MS,
    }),

  repairBroadcastTemplateHeaderAndRetry: (id, data = {}) =>
    api.post(`/broadcasts/${id}/repair-template-header-retry`, data, {
      timeout: LONG_TIMEOUT_MS,
    }),
  ...
  createBroadcast: (data) =>
    api.post("/broadcasts", data, { timeout: LONG_TIMEOUT_MS }),
   ...
   */
  sendBroadcast: (id) =>
    api.post(`/broadcasts/${id}/send`, undefined, { timeout: LONG_TIMEOUT_MS }),
   ...
   */
  cancelBroadcast: (id) =>
    api.post(`/broadcasts/${id}/cancel`, undefined, {
      timeout: LONG_TIMEOUT_MS,
    }),
   ...
   */
  pauseBroadcast: (id) =>
    api.post(`/broadcasts/${id}/pause`, undefined, {
      timeout: LONG_TIMEOUT_MS,
    }),
   ...
   */
  resumeBroadcast: (id) =>
    api.post(`/broadcasts/${id}/resume`, undefined, {
      timeout: LONG_TIMEOUT_MS,
    }),
  ...
  checkScheduledBroadcasts: () =>
    api.post("/broadcasts/check-scheduled", undefined, {
      timeout: LONG_TIMEOUT_MS,
    }),
   ...
   */
  syncBroadcastStats: (id) =>
    api.post(`/broadcasts/${id}/sync-stats`, undefined, {
      timeout: LONG_TIMEOUT_MS,
    }),
   ...
   */
  deleteBroadcast: (id) =>
    api.delete(`/broadcasts/${id}`, { timeout: LONG_TIMEOUT_MS }),
   ...
   */
  getAnalytics: (params = {}) =>
    api.get("/analytics", { params, timeout: LONG_TIMEOUT_MS }),

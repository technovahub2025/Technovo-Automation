...
const isLocalHostname = (hostname = "") => /^(localhost|127\.0\.0\.1)$/i.test(toCleanString(hostname));

const isLocalUrl = (value) => {
  try {
    return isLocalHostname(new URL(toCleanString(value)).hostname);
  } catch {
    return false;
  }
};
...
export const resolveApiBaseUrl = ({ override } = {}) => {
  ...
  const browserHostname = typeof window === "undefined" ? "" : toCleanString(window.location.hostname);
  ...
  if (explicitBase && (browserIsLocal || !isLocalUrl(explicitBase))) return explicitBase;

  const explicitApiUrl = normalizeApiBaseUrl(import.meta.env.VITE_API_URL);
  if (explicitApiUrl && (browserIsLocal || !isLocalUrl(explicitApiUrl))) return explicitApiUrl;
